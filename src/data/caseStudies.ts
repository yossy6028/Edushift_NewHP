...
export const YS_CASE = {
    ...
    disclaimer: `本事例は個別の実績であり、成果を保証するものではありません。問い合わせ件数はHPフォーム経由のメール通知をスレッド単位で集計し、営業目的のメールを除外しています（${AS_OF}時点）。前年同期（2025年6月・7月）のHPフォーム問い合わせは0件でした。問い合わせ増加には時期や発信活動など他の要因が影響している可能性があり、リニューアル単独の効果を示すものではありません。`,
} as const;
